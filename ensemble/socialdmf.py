...
train = pd.read_csv(os.path.join(data_dir, "train.csv")).iloc[:, :3]
...
# 创建 social 评分
social = pd.read_csv(os.path.join(data_dir, "user_list.csv"))
social = list(social.sort_values('user_id')['friends'].values)
social = [eval(string) for string in social]
social.insert(0, [])

sparse_matrix = torch.sparse_coo_tensor(train1_data.t(), train1_targets, torch.Size((NUM_USERS, NUM_ITEMS))).to_dense().cpu()

social_ratings = []
for u, i, r in train[['uid', 'iid', 'rating']].values:
    friends = social[int(u)]
    if len(friends) == 0:
        social_ratings.append(0)
        continue
    i_indices = len(friends)*[i]
    fri_ratings = sparse_matrix[friends, i_indices]
    result = remove_zeros(fri_ratings)
    
    if result.numel() == 0:
        social_ratings.append(0)
        continue
    social_ratings.append(torch.mean(result).item())

social_targets = torch.tensor(social_ratings, dtype=torch.float).to(device)
train_targets = alpha*train_targets + (1-alpha)*social_targets
